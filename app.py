...
Main Flask Application - PostgreSQL Version (psycopg v3)
"""
...
NEWS_API_KEY = os.environ.get(
    "NEWS_API_KEY",
    "4d785e694a43460c8b5acbd9e538ce2e"
)
NEWS_API_URL = "https://newsapi.org/v2/"

DATABASE_URL = os.environ.get(
    "DATABASE_URL",
    "postgresql://dailynest_user:iQYbbTddM6bBY4KfLy1tDW5296PZi5k2@dpg-d6h9ee450q8c73af3jsg-a.oregon-postgres.render.com/dailynest"
)
...
def get_fallback_news():
    ...
    return [
        {
            "title": "DailyNest is Ready!",
            "description": "Add your NewsAPI key to see live news.",
        ...
        },
        {
            ...
            "description": "From AI to quantum computing.",
        ...
        },
        {
            ...
            "description": "Global markets continue upward.",
...
@app.route("/")
def home():
    trending = fetch_news("general", page_size=9)
    tech = fetch_news("technology", page_size=3)
...
def dashboard():
    ...
    prefs = [p.strip() for p in user["preferences"].split(",") if p.strip()] or ["general"]

    news_by_category = {
        cat: fetch_news(cat, page_size=6) for cat in prefs
    }
...
def preferences():
    with get_db() as conn:
        ...
        if request.method == "POST":
            selected = request.form.getlist("categories") or ["general"]
            telegram_id = request.form.get("telegram_id", "").strip()

            with conn.cursor() as cur2:
                cur2.execute(
                    "UPDATE users SET preferences=%s, telegram_id=%s WHERE id=%s",
                    (",".join(selected), telegram_id, session["user_id"]),
                )
            conn.commit()
...
def category(cat):
    ...
    articles = fetch_news(cat, page_size=12)
...
def search():
    ...
    articles = fetch_news(query=query, page_size=12) if query else []
    return render_template("search.html", articles=articles, query=query)


@app.route("/api/news/<category>")
def api_news(category):
    articles = fetch_news(category, page_size=5)
    return jsonify([
        {
            "title": a.get("title", ""),
