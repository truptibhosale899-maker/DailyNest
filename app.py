...
app = Flask(__name__)
app.secret_key = "dailynest_secret_key_2025"
...
def preferences():
    ...
    if request.method == "POST":
        ...
        if not selected:
            selected = ["general"]
        cursor2 = conn.cursor()
        cursor2.execute("UPDATE users SET preferences = %s, telegram_id = %s WHERE id = %s", (",".join(selected), telegram_id, session["user_id"]))
        conn.commit()
        conn.close()
